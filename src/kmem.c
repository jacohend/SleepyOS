//from James Molloy's kernel tutorials
//with my changes to kmalloc
...
//allocates memory. optionally page-aligned. 
u32int kmalloc(u32int size, int page_align, u32int *phys){
    if (kheap != 0){
        void *addr = alloc(size, (u8int) page_align, kheap);
        if (phys != 0) {
            page_properties *page = get_page((u32int)addr, 0, kernel_directory);
...
//pushes segments of memory apart
...
//contract memory space, after freeing, for example
...
//finds freed memory holes created by dynamic memory
...
//frees memory, deallocates heap memory
