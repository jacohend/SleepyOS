//modified from a small posix-complient fs by molloy's tutorials
//it cannot yet write new files or new directories only read/modify
#include "initrd.h"
...
//read data from file node structure
...
//write buffer to file node
static u32int initrd_write(fs_node *node, u32int offset, u32int size, u8int *buffer){ 
    int sz = 0;
    initrd_file_header fheader = file_headers[node->inode];    
    sz = fheader.length;
...
//reads files out of a directory
...
//mount fs, mount root nodes, mount directories, populate file structs from
//initrd module that was written to disk prior to boot
